...
//    rdd.filter(_ % 2 ==0).filter(_ > 2).foreach(println)
//    rdd.filter(x => x % 2 ==0 &&  x > 2).foreach(println)
//
//    sc.parallelize(1 to 20, 4).glom().foreach(println)
//
//    sc.makeRDD(1 to 20).sample(false, 0.2).foreach(println)
//
//    val a = sc.makeRDD(List("ruoze","pk","xingxing","jepson"))
//    val b = sc.makeRDD(List(30,31,60,32))
//    a.zip(b).foreach(println)
//
//    sc.makeRDD(List(("ruoze",30),("jepson",18))).mapValues(_+2).foreach(println)
//
//    sc.makeRDD(List("dog","tiger","lion","cat","owl"))
//        .map(x => (x.length, x))
//        .flatMapValues("-"+_+"=").foreach(println)
//
...
//    left.join(right)
//    left.cogroup(right).foreach(println)
...
//
//    sc.makeRDD(List("拜仁慕尼黑","大巴黎")).cartesian(sc.makeRDD(List("胜","平","负"))).foreach(println)
//
//    sc.makeRDD(List("dog","tiger","lion","cat","owl")).keyBy(_.length).foreach(println)
//
    sc.makeRDD(List("a", "a", "a", "b", "b", "c")).groupBy(x => x).mapValues(_.size).foreach(println)
//    sc.makeRDD(1 to 9).groupBy(x=>{
//      if(x%2==0) "even" else "odd"
//    }).foreach(println)
//
//    // (String, Iterable[Int])
//    sc.makeRDD(List(("a", 1), ("b", 2), ("c", 3), ("a", 80))).groupByKey().mapValues(_.sum).foreach(println)
//
//    // (a,<1,80>) (b,<2>) (c,<3>)  ==> (String, Int)
//    sc.makeRDD(List(("a", 1), ("b", 2), ("c", 3), ("a", 80))).reduceByKey((x,y)=>x+y).foreach(println)
//
...
//
//    sc.makeRDD(List(3,4,5,6,7,8,8)).distinct().foreach(println)
//
//    /**
//     * TODO...
//     * 1) 8 ==> (8,null)   8 ==> (8,null)
//     * 2) reduceByKey((a,b)=>a)
//     */
//    sc.makeRDD(List(3, 4, 5, 6, 7, 8, 8)).map(x => (x, null)).reduceByKey((a, b) => a).map(_._1).foreach(println)
//
//
//    // 链式编程
//
